...
class RequestDecisionBottomSheet extends StatefulWidget {
  ...
  static void show(BuildContext context, RequestModel request) {
    ...
    if (medicineState is MedicinesLoaded) {
      medicine = medicineState.medicines.where((m) => m.id == request.medicineId).firstOrNull;
    }
    ...
    showModalBottomSheet(
      ...
      builder: (_) => RequestDecisionBottomSheet(request: request, medicine: medicine),
    );
  }

  @override
  State<RequestDecisionBottomSheet> createState() => _RequestDecisionBottomSheetState();
}

class _RequestDecisionBottomSheetState extends State<RequestDecisionBottomSheet> {
  int _boxes = 0;
  int _strips = 0;

  bool get _isBoxEnabled => widget.medicine?.unit.isBox ?? true; // fallback to true
    ...
    return Container(
      ...
      child: SafeArea(
        ...
        children: [
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      Text(widget.request.patientName, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
                      const SizedBox(height: 4),
                      Text("${l10n.nationalId}: ${widget.request.patientNationalId}", style: TextStyle(color: theme.textTheme.bodySmall?.color, fontSize: 12)),
                    ],
                  ),
                ),
              ],
            ...
            child: Column(
              ...
              children: [
                  ...
                  children: [
                    ...
                    Expanded(
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text(widget.request.medicineName, style: const TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
                          const SizedBox(height: 4),
                          Text(widget.request.reason, style: TextStyle(color: theme.textTheme.bodySmall?.color, fontSize: 13)),
                        ],
                      ),
                    ),
                  ],
                ...
                if (widget.request.prescriptionPath != null) ...[
                  const SizedBox(height: 16),
                  Text(l10n.prescription, style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 13)),
                  const SizedBox(height: 8),
                  GestureDetector(
                    onTap: () => _showFullScreenImage(context, widget.request.prescriptionPath!),
                    child: ClipRRect(
          ...
          // Available Stock
          Text(l10n.decisionPanel, style: theme.textTheme.titleMedium?.copyWith(fontWeight: FontWeight.w800)),
          ...
          if (_isBoxEnabled) ...[
            _buildCounterRow(
              label: l10n.boxes,
              value: _boxes,
              onDecrement: () => setState(() { if (_boxes > 0) _boxes--; }),
          ...
          if (_isStripEnabled) ...[
            _buildCounterRow(
              label: l10n.strips,
              value: _strips,
              onDecrement: () => setState(() { if (_strips > 0) _strips--; }),
              onIncrement: () => setState(() {
              ...
              Expanded(
                child: OutlinedButton(
                  onPressed: () {
                     ...
                     _showRejectBottomSheet(context, widget.request.id, bloc);
                  },
                  ...
                  child: Text(l10n.reject, style: const TextStyle(color: AppColors.error, fontWeight: FontWeight.bold, fontSize: 16)),
                ),
              ),
              const SizedBox(width: 16),
              Expanded(
                child: ElevatedButton(
                  onPressed: (_boxes == 0 && _strips == 0) ? null : () {
                     context.read<RequestBloc>().add(RequestApproveRequested(
                  ...
                  child: Text(l10n.approve, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                ),
              ),
            ],
          ),
...
void _showRejectBottomSheet(BuildContext parentContext, String requestId, RequestBloc bloc) {
                   ...
                   children: [
                      ...
                      ElevatedButton(
                         onPressed: selectedReason == null ? null : () {
                            final reason = selectedReason == l10n.otherReason ? otherCtrl.text : selectedReason;
                            if (selectedReason == l10n.otherReason && otherCtrl.text.trim().isEmpty) return;
                            
                            bloc.add(RequestApproveRequested(
                               requestId: requestId,
                         ...
                         child: Text(l10n.confirmRejection, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      ),
                   ],
                 ),
              ),
